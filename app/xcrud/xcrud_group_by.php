...
class Xcrud
{
            ...
            foreach ($this->inner_table_instance as $inst_name => $field)
            {
                if (isset($this->result_row[$field]))
                {
                    ...
                    $this->nested_rendered[$inst_name] =
                        ...
                        36) . '">' . $instance->render('list') . '</div></div></div>';
                }
            }
        }
    ...
    protected function _cell_attrib($field, $value, $order, &$row, $is_sum = false, $row_color = false, $row_class = false, $col_span = false)
    {
        $attr = array();
            ...
            foreach ($this->result_list as $key => $row)
            {
                ...
                if (isset($this->highlight_row))
                {
                    foreach ($this->highlight_row as $params)
                    {
						...
						//$rowArr[$params['field']] = $params['value'];
						//array_push($var,$row[$params['field']]);	
                    ...
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'right')
                {
                     ...
                     $out .= $this->close_tag($item);
					  ...
					  /*$out ="<div class='btn-group'><button type='button' class='btn btn-primary'>Sony</button>
					 ...
					 $out .= "<button id='btnGroupVerticalDrop$cnt' type='button' class='btn btn-secondary dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>";
					 $out .= " Dropdown </button>";
					 $out .= "<div class='dropdown-menu' aria-labelledby='btnGroupVerticalDrop$cnt' x-placement='bottom-start' style='position: absolute; transform: translate3d(0px, 38px, 0px); top: 0px; left: 0px; will-change: transform;'>";
					 $out .= "<a class='dropdown-item' href='#'>Dropdown link</a>";
					 $out .= "<a class='dropdown-item' href='#'>Dropdown link</a></div></div></div>";*/
            ...
            {            	
	            ...
	            {                	
					  ...
					  if($field == $this->group_by_columns[0]['table'] . "." . $this->group_by_columns[0]['field']){
						  ...
						  $main_field = $field;
					      $out .= $this->open_tag($row_tag, 'xcrud-row') . $this->open_tag($item, '', array('colspan' => $j)) . $main_value .
					      $this->close_tag($item) . $this->close_tag($row_tag);
						  break 1;
	            ...
	            foreach ($this->result_list as $key => $row)
	            {
					...
					echo $row[$main_field] . "-" . $main_value  . "<br>";
					if($main_value == $row[$main_field]){
						 $j = 0;
	                $row_color = false;
	                $row_class = '';
	                if (isset($this->highlight_row))
